% question 2: running algorithm for n times
clc;
clear;
...
% multiple times run and storing values
all_records = [];  
...
for r = 1:runs
    ...
    while t >= T_min && nfe <= MaxNFE
        for i = 1:i_max
            ...
            if mod(nfe,100) == 0
                record(end+1) = best;   % end+1 = append to next position in the array
            end
...
disp('Average best objective values across runs:');
disp(avg_record);
...
% Plot
figure;
plot(100:100:100*length(avg_record), avg_record,'LineWidth',2);
